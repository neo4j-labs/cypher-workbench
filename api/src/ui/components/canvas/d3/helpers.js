...
export function setNewNodeLabelPosition (nodeLabel, x, y, dataModelInstance, canvasAttr) {
    var position = {x: x, y: y};
    //console.log('canvasAttr');
    //console.log(canvasAttr);
    if ((x === undefined || x === null) || (y === undefined || y === null)) {
        position = findOpenPosition(dataModelInstance, canvasAttr);
    }
    nodeLabel.display.x = position.x;
    nodeLabel.display.y = position.y;
}

function findOpenPosition (dataModelInstance, canvasAttr) {
    ...
    while (cy < (height - bottomMargin)) {
        ...
        while (cx < (width - rightMargin)) {
            ...
            var isOpen = !(intersectsWithAnyNode(dataModelInstance, cx, cy, defaultCircleRadius));
            if (isOpen) {
                foundOpening = true;
                break;
            }
...
export function rectanglesIntersectOkToTouch(minAx, minAy, maxAx, maxAy, minBx, minBy, maxBx, maxBy ) {
    var aLeftOfB = maxAx <= minBx;
    var aRightOfB = minAx >= maxBx;
    var aAboveB = minAy >= maxBy;
    var aBelowB = maxAy <= minBy;
...
function intersectsWithAnyNode (dataModelInstance, cx, cy, r) {
    var nodeLabelArray = dataModelInstance.getNodeLabelArray();
    return nodeLabelArray.some(nodeLabel =>
        circlesIntersect(cx, cy, r, nodeLabel.display.x, nodeLabel.display.y, nodeLabel.display.radius)
    );
}
