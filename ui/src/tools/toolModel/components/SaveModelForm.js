...
export class SaveModelForm extends Component {
    ...
    setValue = (e) => {
        ...
        if (mode !== SAVE_MODE.EXISTS || (mode === SAVE_MODE.EXISTS && SecurityRole.canEdit())) {
            this.props.recordModelMetadataChanges(e.target.id, e.target.value);
        }
    }
    ...
    render () {
        ...
        var { modelInfo } = this.props;
        var { loading, title, description, notes, customers, authors, tags, isPublic } = modelInfo;
        ...
        return (
            <Dialog open={this.props.open} onClose={this.props.onClose} fullWidth={true} maxWidth={this.props.maxWidth} >
                <DialogTitle id="alert-dialog-title">{"Model Information"}</DialogTitle>
                <DialogContent>
