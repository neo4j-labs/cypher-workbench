...
const SlateCypherEditorBlock = ({ cypherBlockKey, dataProvider, cypherSuiteBuilder }) => {
  const ref = useRef(null);
  const initialValue = dataProvider.getCypherStatement();
 ...
 const chars = popupWords.filter(c =>
  ...
  const onPaste = (event) => {
    if (SecurityRole.canEdit()) {
      ...
      if (!plainText) {
        ...
        if (cypherSuiteBuilder) {
          cypherSuiteBuilder.cypherStatementUpdated({
  ...
  const handleAutocomplete = () => {
    ...
    if (selection && Range.isCollapsed(selection)) {
        ...
        var localVariables = []; // TODO: get this from parsing?
        ...
        if (beforeText === '') {
          ...
          switch (parseResults.promptType) {
            ...
            case PROMPT_TYPE.VariableProperty:
              ...
              variableProperties = []; // TODO: get this from somewhere?
  ...
  return (
    <>
        ...
        <Slate
            ...
            onChange={value => {
                setValue(value);
                dataProvider.setCypherStatement(value);
                if (cypherSuiteBuilder.cypherStatementUpdated) {
                  cypherSuiteBuilder.cypherStatementUpdated({
                    cypher: dataProvider.getCypher(),
                    cypherBlockKey
                  })
            ...
            <Editable
                ...
                */
                onPaste={onPaste}
                onFocus={() => {
                  if (cypherSuiteBuilder && cypherSuiteBuilder.onBlockFocused) {
                    cypherSuiteBuilder.onBlockFocused(cypherBlockKey);
                  }
                }}
